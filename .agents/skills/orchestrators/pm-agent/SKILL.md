...
| 4.5 API 契約 | `docs: add api contract for <feature>` |
...
詳細設計承認後・実装前に API 契約を確定する。backend 未割当予定（API なし）の場合はスキップして Phase 5 へ。

[create-api-contract](../../abilities/create-api-contract/SKILL.md) → [review-api-contract](../../abilities/review-api-contract/SKILL.md)

- 自動差し戻し最大 2 回
- hotfix モードでは該当ユースケースの契約差分のみ更新可
- 人間承認後 `docs: add api contract for <feature>` をコミット → Phase 5
...
- **verify-run が不合格の場合は review-code を実行しない**（implement へ差し戻し）
...
[create-acceptance](../../abilities/create-acceptance/SKILL.md) → [review-acceptance](../../abilities/review-acceptance/SKILL.md)

- 自動差し戻し最大 1 回
- 人間承認後 `docs: add acceptance report for <feature>` をコミットして完了報告
- 未達があれば [差し戻し連鎖](#差し戻し連鎖ルール) に従い該当 Phase へ戻す
